...
var pilihan_jawaban = ["me", "ba", "mem", "ca"]
var jawaban_benar = "membaca"  # Jawaban yang benar
...
func _on_ceklis_2_pressed():
	...
	if jawaban_gabungan == jawaban_benar:
		...
		update_sprite("benar")
		get_tree().change_scene_to_file("res://winner_2_Soal_1.tscn")  # Pindah ke scene berikutnya jika benar
	else:
...
#func _on_time_up():
	#feedback_label.text = "Waktu habis!"
	#update_sprite("salah")
	#get_tree().change_scene_to_file("res://gameover_2.tscn")  # Scene pindah jika waktu habis
#
## Fungsi yang dipanggil setiap kali timer timeout untuk mengurangi waktu dan memperbarui label
#func _on_Timer_timeout():
	#var remaining_time = int(timer.wait_time - timer.time_left)
	#timer_label.text = str(remaining_time)
#
	#if remaining_time <= 0:
		#_on_time_up()
